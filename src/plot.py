...
############    Show Function     ############
##############################################
def show(custom_terminal_size = None):
    """\nIt prints the final figure on terminal.\n"""
    _size_max(custom_terminal_size)
...
##############################################
def _size_max(custom_terminal_size = None):
    par.width_max, par.height_max = terminal_size(custom_terminal_size)
    par.height_max -= 2
...
def terminal_size(custom_terminal_size = None):
    """\nIt returns the terminal size\n"""
    if custom_terminal_size is None:
        if par.nocolor:
            return [100, 45]
        else:
            return list(_os.get_terminal_size())
    else:
        return custom_terminal_size
